...
class Passkey(Base):
    """A WebAuthn credential registered by a user. A user can have several, e.g. one
    per device or password manager."""
    ...
    # Unsigned 32-bit counter kept by the authenticator (0 for synced passkeys, which
    # don't count).
    sign_count: Mapped[int] = mapped_column(BigInteger)
    # How the browser can reach the authenticator (e.g. internal, hybrid, usb), reported
    # at registration.
...
class WebAuthnChallenge(Base):
    """A challenge issued by a `*-challenge` endpoint. The matching verify endpoint
    deletes it, so it works once."""
