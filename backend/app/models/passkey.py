...
class Passkey(Base):
    """A WebAuthn credential registered by a user. A user can have several, e.g. one per device
    or password manager."""
    ...
    # How the browser can reach the authenticator (e.g. internal, hybrid, usb), reported at
    # registration.
...
class WebAuthnChallenge(Base):
    """A challenge issued by a `*-challenge` endpoint. The matching verify endpoint deletes it,
    so it works once."""
    ...
    # Stored as the enum's values in a VARCHAR; a native Postgres enum type would need its own
    # migration steps.
