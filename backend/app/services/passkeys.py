"""Passkey (WebAuthn) registration and sign-in. Each ceremony has two steps: issue a challenge
and the options the browser needs, then verify the credential the browser returns for that
challenge."""
...
from webauthn.helpers import options_to_json_dict, parse_client_data_json
...
class PasskeyVerificationError(Exception):
    """The credential answers an unknown, expired or used challenge, or fails verification."""


def start_registration(db: Session, *, email: str) -> RegistrationOptions:
    ...
    return RegistrationOptions.model_validate(options_to_json_dict(registration_options))
...
def finish_registration(db: Session, registration: RegistrationResponse) -> User:
    ...
    # Read before committing: a failed commit expires the challenge row, which no longer exists.
    ...
    try:
        # The unique index on email is the source of truth: of two sign-ups racing for one email,
        # one gets here.
        db.commit()
...
def start_authentication(db: Session) -> AuthenticationOptions:
    """Return request options for signing in. They name no credentials, so the browser offers every
    passkey it has for this site and the user doesn't type an email."""
    ...
    return AuthenticationOptions.model_validate(options_to_json_dict(authentication_options))
...
def finish_authentication(db: Session, authentication: AuthenticationResponse) -> User:
    ...
    try:
        ...
        passkey = db.scalar(select(Passkey).where(Passkey.credential_id == authentication.raw_id))
        if passkey is None:
            raise PasskeyVerificationError("Unknown credential")
        # A discoverable credential reports the user it was created for; it must be the passkey's
        # owner.
...
def _consume_challenge(
    ...
    """Find and delete the challenge the credential answers. It's deleted even if verification then
    fails, so every challenge gets exactly one attempt."""
