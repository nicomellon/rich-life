...
class Settings(BaseSettings):
    """Configuration read from environment variables only (twelve-factor). Loading a .env file is
    the job of whatever starts the process, e.g. `uv run --env-file`, never of the app itself."""

    model_config = SettingsConfigDict(extra="ignore")

    # Set by the build or deploy pipeline (e.g. a git tag or commit SHA); shown in /docs and
    # /health.
    app_version: str = "dev"
    ...
    # Passkeys are bound to the relying party ID: the domain the web app is served from, without
    # scheme or port. Changing it in production invalidates every registered passkey.
